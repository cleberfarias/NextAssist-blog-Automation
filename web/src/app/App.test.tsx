...
describe("App shell", () => {
  ...
  it("renders all 9 sidebar items and navigates between routes", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn((input: RequestInfo | URL) => {
      ...
      if (url.includes("/api/analytics/overview")) {
        return Promise.resolve({
          ...
          json: async () => ({
            rangeDays: 30,
            visits: { value: 0, changePct: null },
            leads: { value: 0, changePct: null },
            conversionRate: { value: 0, changePoints: null },
            customers: { value: 0, changePct: null },
            series: [],
            leadSources: [],
          }),
        });
    ...
    expect(await screen.findByText("Visitas no site")).toBeInTheDocument();
  });
});
