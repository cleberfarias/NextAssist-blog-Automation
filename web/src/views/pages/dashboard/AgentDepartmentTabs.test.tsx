...
describe("AgentDepartmentTabs", () => {
  ...
  it("alterna entre o departamento Agent Office (5 agentes) e Equipe de conteúdo (8 agentes)", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn((input: RequestInfo | URL) => {
      ...
      if (url.includes("/api/content-calendar")) return Promise.resolve({ ok: true, json: async () => [] });
    ...
    expect(screen.queryByText("Fluxo da sua equipe de 8 agentes")).not.toBeInTheDocument();

    await userEvent.click(screen.getByRole("tab", { name: /Equipe de conteúdo/ }));
    expect(await screen.findByText("Fluxo da sua equipe de 8 agentes")).toBeInTheDocument();
