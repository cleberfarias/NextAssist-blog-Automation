...
function validateWorkspaceShape(id: string, value: unknown): MarketingWorkspace {
  ...
  if (w.videoStrategy !== undefined) {
    ...
    if (videoRaw.fallback !== "none") fail('"videoStrategy.fallback" precisa ser "none" para HeyGen MCP');

    const heygenRaw = integrationsRaw.heygen === undefined ? null : requireObject(integrationsRaw.heygen, "integrations.heygen");
    if (!heygenRaw || heygenRaw.transport !== "mcp" || heygenRaw.auth !== "oauth") {
      fail('videoStrategy.provider="heygen-mcp" exige integrations.heygen com transport="mcp" e auth="oauth"');
    }
  }
